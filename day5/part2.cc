...
void move (int num_to_move, int source, int destination, std::vector<std::stack<char>> &vector) {
    std::stack<char> stack;
    for(int i = 0; i < num_to_move; i++){
        stack.push(vector[source].top());
        vector[source].pop();
    }
    for(int i = 0; i < num_to_move; i++){
        vector[destination].push(stack.top());
        stack.pop();
    }
}
...
int main() {
    ...
    while(getline(file, line)){
        ...
        move(std::get<0>(nums), std::get<1>(nums)-1 , std::get<2>(nums)-1, config);
    }
