...
constexpr int DESIRED_SPACE = 30000000;
...
int main () {
    ...
    const int free_space = DISK_SIZE - root_dir.calculate_size();
    const int size_to_free = DESIRED_SPACE - free_space; 
    int closest = root_dir.calculate_size();
    for(int i = 0; i < nodes.size(); i++){
        if(nodes[i].second > size_to_free && nodes[i].second < closest){
            closest = nodes[i].second;
        }
    }
